...
/**
 * Makes Labels from spreadsheet rows.
 */
function makeLabels() {
  //Our logo
  var image  = UrlFetchApp.fetch("http://ictensw.org.au/resources/Pictures/logo/smallLogo.png");
  
  //Open and set up the new document to contain a table that we will add to
  var newDoc = DocumentApp.create("LabelsPleaseWork");
  ...
  body.setMarginLeft(13)	
  body.setMarginRight(72)	
  body.setMarginTop(13)	
  ...
  var dataRange = dataSheet.getRange(2, 1, dataSheet.getMaxRows() - 1, 34);

  var templateSheet = ss.getSheets()[1];
  var labelTemplate = templateSheet.getRange('A1').getValue();
  ...
  for (var i = 0; i < objects.length; ++i) {
    ...
    // Generate a personalized label.
    // Given a template string, replace markers (for instance ${"First Name"}) with
    // the corresponding value in a row object (for instance rowData.firstName).
    var labelText = fillInTemplateFromObject(labelTemplate, rowData);

    //Need to organise a new doc and append the table rows and columns
    if (i%2==0){
    //first in row
      var tr = table.appendTableRow();
      tr.setMinimumHeight(108)
      
    }
    //either make a new row or put the label to the right
    //second in row
    var td = tr.appendTableCell('');
    td.insertParagraph(0, labelText);
    td.insertImage(0,image.getBlob());
...
function fillInTemplateFromObject(template, data) {
  ...
  for (var i = 0; i < templateVars.length; ++i) {
    ...
    label = label.replace(templateVars[i], variableData || '');
  }
...
2. Formatting of size of table so that it works with Avery stickers
3. Add an image logo
4. Work out how to run it from a button or something
